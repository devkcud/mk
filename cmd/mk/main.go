...
func main() {
	...
	flagLogLevel := flag.IntP("loglevel", "l", 2, "Set log level (0: none, 1: error, 2: warn, 3: info, 4: all)")
	...
	if *flagPreview {
		if _, err := os.Stat(previewFolder); os.IsNotExist(err) {
			utils.CreateDir(previewFolder)
		} else {
			os.RemoveAll(previewFolder)
		}

		os.Chdir(previewFolder)
	}
